...
connectDB()
    .then(() => {
    sessionCleanUpQueue.add("cleanup-expired-sessions", {}, {
        repeat: {
            cron: "0 * * * *",
        },
    });
    ...
    sessionCleanUpQueue.on("completed", (job, result) => {
        console.log(`Job ${job.id} completed. Processed ${result.length} sessions:`, `${result.filter((r) => r.status === "deleted").length} deleted,`, `${result.filter((r) => r.status === "failed").length} failed`);
    });
...
//# sourceMappingURL=sessionCleanUpQueue.js.map